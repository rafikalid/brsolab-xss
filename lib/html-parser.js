...
function parse(html, onTag){
	...
	var scriptStyle	= false, scriptStyleLen, rmTagBody = false, rmTagBodyCount=0;
				...
				// closing tag
					...
					try{
						...
						// remove tag body
							...
							if(rmTagBody === tagNameLC){
								if(str.startsWith('</')){// end tag
									if(rmTagBodyCount > 0) --rmTagBodyCount;
									else rmTagBody = false;
									console.log('--- end tag: ', tagNameLC)
								}
								else ++rmTagBodyCount;
							}
						}
